...
** NAT
*** 内网与外网
网络服务器中，外网的服务器无法定位到内网服务器，除非外网服务器将网络服务器设置为网关，因此有两种方法: 
1. NAT(Network Address Translation): 用于隐藏本地网络主机，提高安全性在网络层(更改IP)和传输层(更改端口)实现
2. proxy：代理服务器(网关)，在应用层(区分代理的协议)实现

*** NAT类型
- SNAT: 只修改请求报文的源地址，在POSTROUTING链的nat表中转换，其响应报文会自动根据NAT表修改目标地址
- DNAT：只修改请求报文的目标地址(不然目标地址为本主机)，在PREROUTING链的nat表中转换，其响应报文会自动根据NAT表修改源地址
- FULLNAT
