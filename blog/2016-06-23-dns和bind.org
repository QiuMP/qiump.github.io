...
记录了DNS的相关内容，包括DNS的解析、DNS的服务器、DNS的资源记录RR，同时简单介绍了如何使用bind来建立DNS服务器，包括主DNS服务器、从DNS服务器、缓存DNS服务器、转发DNS服务器。以及介绍了DNS相关命令的使用，例如dig、host、nslookup，和DNS服务器管理工具rndc、DNS服务器压力测试工具queryperf
...
- 一般将需求递归给DNS服务器，DNS服务器从根服务器开始进行迭代查询
- 特殊查询结果
...
- allow-update {}: 允许更新区域数据库中的内容，一般关闭(使用none)
** view
- view用于为不同的客户端提供不同的zone解析
